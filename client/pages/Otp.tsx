        ...
        <div className="mt-5 flex justify-between gap-3">
          {code.map((v,i)=> (
            <input key={i} value={v} maxLength={1} onChange={(e)=>{
              const next=[...code]; next[i]=e.target.value.replace(/[^0-9]/g,""); setCode(next);
            }} className="h-14 w-14 rounded-xl border bg-neutral-100 text-center text-xl outline-none focus:bg-white" />
          ))}
        </div>
        <div className="mt-6 space-y-3">
          <Button className="h-12 w-full rounded-full" disabled={!canSubmit} onClick={()=>nav("/vehicle")}>Verify Code</Button>
          <Button variant="outline" className="h-12 w-full rounded-full">Send Again</Button>
        </div>
      </div>
    </Layout>
  );
