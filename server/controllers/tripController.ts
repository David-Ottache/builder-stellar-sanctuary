...
export const endTrip: RequestHandler = async (req, res) => {
  try {
    ...
    const fee =
      Number.isFinite(bodyFee) && bodyFee >= 0 ? Math.round(bodyFee) : 0;
    if (!id) return res.status(400).json({ error: "id required" });
    if (!isInitialized()) {
      const init = await initializeFirebaseAdmin();
      if (!init.initialized)
    ...
    if (data.status === "completed")
      return res.json({ trip: { id: doc.id, ...data } });

    const pmRaw =
      req.body && (req.body as any).paymentMethod
        ? String((req.body as any).paymentMethod).toLowerCase()
        : "";
    const paymentMethod =
      pmRaw === "wallet" || pmRaw === "cash" ? pmRaw : undefined;
      ...
      if (updated.userId)
        await db
          ...
          .add({
            ...
            body: `Trip ${updated.id} completed. Total: ₦${Number(updated.fee || 0).toLocaleString()}`,
            ts,
            read: false,
          });
      if (updated.driverId)
        await db
          ...
          .add({
            ...
            title: "Trip completed",
            body: `Trip ${updated.id} completed.`,
            ts,
            read: false,
          });
    } catch {}
...
export const averageCost: RequestHandler = async (req, res) => {
  try {
    ...
    const matches = trips.filter(
      (t: any) =>
        typeof t.pickup === "string" &&
        typeof t.destination === "string" &&
        String(t.pickup).toLowerCase().includes(from) &&
        String(t.destination).toLowerCase().includes(to) &&
        Number(t.fee || 0) > 0,
    );
    if (!matches.length) return res.json({ average: null, count: 0 });
    const sum = matches.reduce(
      (acc: number, t: any) => acc + Number(t.fee || 0),
      0,
    );
