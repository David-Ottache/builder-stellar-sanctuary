...
export const listRequests: RequestHandler = async (req, res) => {
  try {
    ...
    const toQ = await db
      ...
      .get()
      .catch(() => ({ docs: [] }) as any);
    const fromQ = await db
      .collection("walletRequests")
      .where("from", "==", userId)
      .limit(100)
      .get()
      .catch(() => ({ docs: [] }) as any);
    const toReq = (toQ.docs || []).map((d: any) => ({
      id: d.id,
      ...(d.data() as any),
    }));
    const fromReq = (fromQ.docs || []).map((d: any) => ({
      id: d.id,
      ...(d.data() as any),
    }));
...
export const deductFunds: RequestHandler = async (req, res) => {
  try {
    ...
    const adminRef = adminUserId
      ? db.collection("users").doc(adminUserId)
      : null;
    ...
    await db.runTransaction(async (t: any) => {
      ...
      const commission =
        driverRef && adminRef ? Math.floor((commissionPercent / 100) * a) : 0;
      ...
      if (driverRef) {
        ...
        try {
          ...
          const body = tripId
            ? `Wallet credited ₦${payout} for trip ${tripId}`
            : `Wallet credited ₦${payout}`;
          t.set(notifRef, {
            userId: driverId,
      ...
      if (adminRef && driverRef && commission > 0) {
        ...
        if (!aDoc.exists) {
          t.set(
            adminRef,
            { id: adminUserId, walletBalance: 0 },
            { merge: true },
          );
        } else {
  ...
  try {
    ...
    try {
      ...
      const fromQ = await db
        ...
        .get()
        .catch(() => ({ docs: [] }) as any);
      const toTx = (toQ.docs || []).map((d: any) => ({
        id: d.id,
        ...(d.data() as any),
      }));
      const fromTx = (fromQ.docs || []).map((d: any) => ({
        id: d.id,
        ...(d.data() as any),
      }));
